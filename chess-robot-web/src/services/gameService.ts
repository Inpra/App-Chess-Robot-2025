...
class GameService {
  ...
  /**
   * Test start game (guest mode, no auth required)
   */
  async testStartGame(): Promise<StartGameResponse> {
    try {
      const response = await fetch(`${this.baseUrl}${GAME_ENDPOINTS.TEST_START}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Failed to start guest game');
      }
      ...
      return {
        ...
        gameTypeCode: data.game_type,
        difficulty: data.difficulty,
        status: data.status,
        message: data.message
      };
    } catch (error) {
      console.error('[GameService] Test start game error:', error);
      throw error;
    }
  }
  ...
  /**
   * Test end game (guest mode)
   */
  async testEndGame(gameId: string): Promise<any> {
    try {
      // Construct URL manually to be safe: /Games/test-end
      const url = `${this.baseUrl}/Games/test-end?gameId=${gameId}`;
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Failed to end guest game');
      }

      return await response.json();
    } catch (error) {
      console.error('[GameService] Test end game error:', error);
